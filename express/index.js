...
app.put('/update/:id', async (req, res) => {
    try {
        ...
        const startTime = Date.now();
        const uploadImage = await pool.query('UPDATE images SET albumId = $1, title = $2, url = $3, thumbnailUrl = $4 WHERE id = $5 RETURNING *', [albumId, title, url, thumbnailUrl, id]);
        const endTime = Date.now();
        const timeTaken = endTime - startTime;
...
app.delete('/delete/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const startTime = Date.now();
        const deleteImage = await pool.query('DELETE FROM images WHERE id = $1 RETURNING *', [id]);
        const endTime = Date.now();
        const timeTaken = endTime - startTime;
